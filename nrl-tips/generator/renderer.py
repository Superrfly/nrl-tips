...
def render_card(card: dict, game_num: int) -> str:
    ...
    # Scoring positions
    home_pos = render_positions(card.get("top_positions_home", []), card.get("home_team", "Home"))
    away_pos = render_positions(card.get("top_positions_away", []), card.get("away_team", "Away"))
    positions_html = ""
...
<head>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .pos-block { background: var(--surface2); border-radius: 8px; padding: 0.6rem 0.75rem; border: 1px solid var(--border); }
    .pos-team { font-size: 11px; font-weight: 600; color: #888; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 6px; }
    .pos-row { display: flex; align-items: center; gap: 6px; margin-bottom: 4px; font-size: 13px; }
    .pos-name { font-weight: 600; min-width: 32px; color: #eee; }
    .pos-scored { color: #639922; flex: 1; }
    .pos-conceded { color: #E24B4A; }
...
def render_positions(positions: list[dict], team_name: str) -> str:
    """Render top 3 scoring positions as a small table."""
    if not positions:
        return ""
    rows = ""
    for p in positions[:3]:
        rows += f"""
        <div class="pos-row">
          <span class="pos-name">{p.get('position', '')}</span>
          <span class="pos-scored" title="Scored">▲ {p.get('scored', 0)}</span>
          <span class="pos-conceded" title="Conceded">▼ {p.get('conceded', 0)}</span>
        </div>"""
    return f"""
    <div class="pos-block">
      <div class="pos-team">{team_name}</div>
      {rows}
    </div>"""
